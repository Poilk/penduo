...
void func_in_loop(int64_t interval_ms) {
  ...
  LOG_INFO << "func_in_loop set: " << interval_ms << " differ: " <<
           Timestamp::differ<Timestamp::TsMicroseconds>(last_mp[interval_ms], now) << std::endl;
...
void func_run_at(){
  ...
  LOG_INFO << "func_run at differ: " <<
           Timestamp::differ<Timestamp::TsMicroseconds>(last, now) << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  loop.run_at(Timestamp::add<Timestamp::TsMilliseconds>(last, 25), func_run_at);
